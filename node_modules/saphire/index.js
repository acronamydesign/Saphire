...
saphire.start = function (app) {
	...
	if (core("saphire-regions")) {
		...
		//render funcion is all hocked up
		var render = saphire.render;
	}
	...
	if (core("saphire-themes")) {
		...
		var markup = saphireThemes.use(app.settings.views, app); //the theme its template overides and markup generation here
	}
	...
	if (core("saphire-admin")) {
		...
		function route() {
			this.unique = function (app, url, renderPath, data) {
		...
		for (purpose in adminRoutes) {
			...
			route.unique(admin, adminRoutes[purpose]["routes"]["url"], app.settings.views + "/admin-index.jade", adminRoutes[purpose])
...
//functions that exist because of the previous check
if (core("saphire-regions")) {
